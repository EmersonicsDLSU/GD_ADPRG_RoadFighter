...
int EnemyAirplane::recentKillCount = 0;


EnemyAirplane::EnemyAirplane(string name, sf::Vector2f position, std::string laneType, std::string emptyGOtype) :
...
void EnemyAirplane::initialize()
{
	...
	sprite->setTextureRect(sf::IntRect(
		TextureManager::getInstance()->parse1.getSheet("frames")[TextureManager::getInstance()->listOfProperties[2].c_str()]["frame"]["x"].GetInt(),
		TextureManager::getInstance()->parse1.getSheet("frames")[TextureManager::getInstance()->listOfProperties[2].c_str()]["frame"]["y"].GetInt(),
		TextureManager::getInstance()->parse1.getSheet("frames")[TextureManager::getInstance()->listOfProperties[2].c_str()]["frame"]["w"].GetInt(),
		TextureManager::getInstance()->parse1.getSheet("frames")[TextureManager::getInstance()->listOfProperties[2].c_str()]["frame"]["h"].GetInt()
	));
	// JSON ABOVE
	//change size
...
{
	APoolable* copyObj = new EnemyAirplane(this->name, position, laneType, emptyGOtype);

	return copyObj;
}

void EnemyAirplane::applyPhysics(Collider* coll)
{
	//access components
