...
int TruckObject::recentKillCount = 0;


TruckObject::TruckObject(string name, sf::Vector2f position, std::string laneType, std::string emptyGOtype) :
	APoolable(name), CollisionListener(), position(position), laneType(laneType), emptyGOtype(emptyGOtype)
{

}

void TruckObject::initialize()
{
	//assign texture
	...
	sprite->setTextureRect(sf::IntRect(
		TextureManager::getInstance()->parse1.getSheet("frames")[TextureManager::getInstance()->listOfProperties[6].c_str()]["frame"]["x"].GetInt(),
		TextureManager::getInstance()->parse1.getSheet("frames")[TextureManager::getInstance()->listOfProperties[6].c_str()]["frame"]["y"].GetInt(),
		TextureManager::getInstance()->parse1.getSheet("frames")[TextureManager::getInstance()->listOfProperties[6].c_str()]["frame"]["w"].GetInt(),
		TextureManager::getInstance()->parse1.getSheet("frames")[TextureManager::getInstance()->listOfProperties[6].c_str()]["frame"]["h"].GetInt()
	));
	// JSON ABOVE
	//change size
	sprite->setScale(sprite->getScale().x * 0.9f, sprite->getScale().y * 1.0f);
	...
	TruckObjectBehavior* behavior = new TruckObjectBehavior("EnemyBehavior");
...
void TruckObject::onRelease()
{

}

void TruckObject::onActivate()
{
	//reset state
	TruckObjectBehavior* behavior = (TruckObjectBehavior*)this->findComponentByName("EnemyBehavior");
...
APoolable* TruckObject::clone()
{
	APoolable* copyObj = new TruckObject(this->name, position, laneType, emptyGOtype);

	return copyObj;
}

void TruckObject::applyPhysics(Collider* coll)
{
	//access components
